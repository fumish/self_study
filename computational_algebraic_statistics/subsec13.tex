...
\begin{frame}
	...
	\end{lemma}
	証明は次ページで行うが、この補題によって極小元によって単項式イデアルは表現できる事がわかる(ただし、極小元の数は有限かどうかはこの補題では分からない)。
\end{frame}
...
\begin{frame}
	ここまでで構成法にも多少なれたと思うので、これをまとめる形でDicksonの補題を証明する
...
\begin{frame} {この節のまとめ}
	...
	\begin{block} {コメント}
		ただし、この方法は単項式イデアルにしか適用できない。また最初に話した連立方程式は、基本的に多項式イデアルである: \\
		例: $\langle xy, x^4 \rangle$は単項式イデアルだが、代数多様体は$\{(0,0)\}$で自明な解しか得られない。\\
		多項式イデアルについては、次節で扱う。
	\end{block}
\end{frame}
